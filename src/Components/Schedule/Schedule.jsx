...
export function Schedule({ state, dispatch }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const response = await fetch("http://localhost:8000/api/v1/allData", {
        method: "GET",
      });
      const newData = await response.json();
      setDataBase(newData.data[0]);
    };
  ...
  //   fetch shorthands
  // const preShifts = dataBase.preShifts
  return (
    <>
      {dataBase && (
        <div className={s.scheduleWrapper}>
          ...
          <div className={s.preShiftsWrapper}>
            ...
            <div>
              ...
              <button
                onClick={() =>
                  dispatch({ type: ACTIONS.TIME, payload: newTime })
                }
              >
                Add new preTime
              </button>
            </div>
            {dataBase.preShifts.map((shift) => (
